...
export async function POST(req: NextRequest, { params }: RouteProps) {
  try {
    ...
    const result = await refundEscrow(id, auth, body.reason);
    ...
    return apiSuccess({ escrow: result.escrow, payment: result.payment });
  } catch (error: any) {
    if (error instanceof AuthError) {
      return apiError(error.message, error.status, error.code);
    }
    console.error("POST /api/escrows/[id]/refund error:", error);
